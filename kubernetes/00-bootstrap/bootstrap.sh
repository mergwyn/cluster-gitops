...
ensure_k3s_profile() {
  ...
  profile="k3s"
  if ! lxc profile list | awk '{print $2}' | grep -qx k3s; then
...
get_ip() {
  lxc list "$1" --format csv -c 4 | grep -v ${VIRTUAL_IP} | head -1 | tr -d \" | cut -d' ' -f1
}

push_k3s_config() {
  local name="$1"

  echo ">>> Pushing k3s config to ${name}"

  lxc file push -p -r k3s-config/* "${name}/etc/rancher/k3s/"
...
bootstrap_cluster() {
  ...
  for ENTRY in "${CLUSTER_NODES[@]}"; do
    ...
    echo ">>> Creating ${NAME}"
    ensure_k3s_profile 
    lxc launch "$IMAGE" "$NAME" --profile "$PROFILE" --profile k3s -c limits.memory="$MEMORY" -c limits.cpu="$CPUS"
  done
  ...
  push_k3s_config "$FIRST_NODE"

  echo ">>> Installing k3s server on first node"
  lxc exec "$FIRST_NODE" -- bash -c "
...
"
  ...
  until lxc exec "$FIRST_NODE" -- /usr/local/bin/k3s kubectl get nodes >/dev/null 2>&1; do
    sleep 2
  done
...
"
  ...
  lxc exec "$FIRST_NODE" -- cat /etc/rancher/k3s/k3s.yaml \
...
clean_cluster() {
  ...
  for ENTRY in "${CLUSTER_NODES[@]}"; do
    parse_entry "$ENTRY"
    if lxc info "${NAME}" &>/dev/null; then
      echo "${NAME} is active, stopping and deleting"
      lxc stop "${NAME}"
      lxc delete "${NAME}"
    else
